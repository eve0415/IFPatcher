...
public class PatchPlantSower extends Patch {
    ...
    public static void tillBlock(final World world, final boolean hoeGround, final ItemStack stack, final BlockPos pos) {
        final FakePlayer player = IndustrialForegoing.getFakePlayer(world);
        if (hoeGround && !ItemStackUtils.isChorusFlower(stack) && !ItemStackUtils.isStackOreDict(stack, "treeSapling")) {
            final ItemStack hoe = new ItemStack(Items.DIAMOND_HOE);
            player.setHeldItem(EnumHand.MAIN_HAND, hoe);
            hoe.getItem().onItemUse(player, world, pos.down(), EnumHand.MAIN_HAND, EnumFacing.UP, 0.5f, 0.5f, 0.5f);
        }
    }
    ...
    protected boolean patch() {
        ...
        for (final ListIterator<AbstractInsnNode> it = work.iterator(); it.hasNext(); ) {
            ...
            if ((insnNode instanceof FieldInsnNode) && ((FieldInsnNode) insnNode).name.equals("hoeGround")) {
        ...
        newInst.add(new FieldInsnNode(GETFIELD, hook, getName("world", "field_145850_b"), "Lnet/minecraft/world/World;"));
        newInst.add(new VarInsnNode(ALOAD, 0));
        ...
        newInst.add(new MethodInsnNode(INVOKESTATIC, hookClass, "tillBlock", "(Lnet/minecraft/world/World;ZLnet/minecraft/item/ItemStack;Lnet/minecraft/util/math/BlockPos;)V", false));
